...
Based on the categories given in Table \ref{tab_fault typles}, we next study these faults and their corresponding MFS to observe whether there exists any distinction difference among different types.


\subsection{The influence of inputs model}
The last one research question is raised from CT itself. It is known that inputs modeling, as the key part of CT \cite{nie2011survey}, significantly influence on the result of  . This question is correlated to the CT. In fact, different building model making different MFS. and different MFS may have different guaranteed code. Take the example in Section \ref{sec:pre}, if we , we even cannot get the correct MFS. Hence, the third research question is:
