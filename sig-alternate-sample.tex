...
To answer this question, we need first give the The characterization and classification of software faults. According to the study \cite{duraes2006emulation}, we decide conduct experiment on the fault types in Table , These faults are classified according to the method \cite{chillarege1996orthogonal}
...
Given the classification, we next need to and then categorized these faults into these types, and observe whether there is difference between them, and if there is, we need to give the specific difference.


\subsection{The influence of inputs model}
The last one arises from CT. This question is correlated to the CT. In fact, different building model making differnet MFS. and differnet MFS may have different guaranteed code.

\textbf{Q3: Is the building model affected  MFS ?}
