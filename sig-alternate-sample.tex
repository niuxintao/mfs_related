...
The last one research question is raised from CT itself. It is known that inputs modeling, as the key part of CT \cite{nie2011survey}, significantly influence on the result of CT. For example, Take the example in Section \ref{sec:pre}, if we use the different model, we even cannot get the correct MFS. The changes of MFS propagates to the guaranteed code, which eventually impacts on correlation between the MFS and the faulty code .  Still for the example, if we change, then,
 
 This question is correlated to the CT. In fact, different building model making different MFS. and different MFS may have different guaranteed code.  
